    ...
    extends TestCase
{
    ...
    {
        final Connector connector = new Connector();
        final LimitingReconnectPolicy policy = new LimitingReconnectPolicy( 1,
                                                                            1 );
        ...
        throws Exception
    {
        ...
        final long now = System.currentTimeMillis();
        assertEqualTime( "time", now, connector.getLastRxTime() );
        assertEquals( "tx vs rx time", connector.getLastTxTime(),
                      connector.getLastRxTime() );
        ...
        throws Exception
    {
        ...
        assertEquals( "isConnected pre connect", false,
        ...
        assertEqualTime( "getLastConnectionTime", now,
                         connector.getLastConnectionTime() );
        assertEquals( "isConnected post connect", true,
                      connector.isConnected() );
        assertEquals( "getConnectionAttempts", 0,
                      connector.getConnectionAttempts() );
        assertEquals( "getConnectionError", null,
        ...
        throws Exception
    {
        final Mock connectorMock = new Mock( ConnectorConnection.class );
        final Exception exception = new Exception();
        connectorMock.expectAndThrow( "doValidateConnection", C.NO_ARGS,
                                      exception );
        connectorMock.expect( "doConnect", C.NO_ARGS );
        final ConnectorConnection connection = (ConnectorConnection)connectorMock.proxy();

        final Mock monitorMock = new Mock( ConnectorMonitor.class );
        monitorMock.expect( "errorValidatingConnection",
    ...
    private void assertEqualTime( final String description,
