...
import org.realityforge.io.EndianUtil;

/**
 *
 * @author <a href="mailto:peter at realityforge.org">Peter Donald</a>
 * @version $Revision: 1.1 $ $Date: 2003-03-24 07:47:06 $
 */
    ...
    extends TestCase
{
    ...
    private static final byte[] LONG_BYTES = new byte[]{85, -95, -82, -58, 0, 0, 0, 0};
    private static final byte[] FLOAT_BYTES = new byte[]{-82, 71, 93, 66};
    private static final byte[] DOUBLE_BYTES = new byte[]{-77, -54, 111, 32, -83, 10, -127, 64};

    private static final byte[] FULL_BYTES = new byte[]{1, 2, 3, 4, 5, 6, 7, 8};
    ...
    public void testSingleSwaps()
    {
        ...
        assertTrue( "Single swap DOUBLE_VALUE",
                    SWAPPED_DOUBLE_VALUE == EndianUtil.swapDouble( DOUBLE_VALUE ) );
    }
    ...
    public void testDoubleSwaps()
    {
        assertEquals( "Double swap SHORT_VALUE",
                      SHORT_VALUE,
                      EndianUtil.swapShort( EndianUtil.swapShort( SHORT_VALUE ) ) );
        ...
        assertEquals( "Double swap INTEGER_VALUE",
                      INTEGER_VALUE,
                      EndianUtil.swapInteger( EndianUtil.swapInteger( INTEGER_VALUE ) ) );
        assertTrue( "Double swap FLOAT_VALUE",
                    FLOAT_VALUE == EndianUtil.swapFloat( EndianUtil.swapFloat( FLOAT_VALUE ) ) );
        assertTrue( "Double swap DOUBLE_VALUE",
                    DOUBLE_VALUE == EndianUtil.swapDouble( EndianUtil.swapDouble( DOUBLE_VALUE ) ) );
    }
    ...
    public void testWriteLongBytes()
    {
        ...
        EndianUtil.writeSwappedShort( data, 0, EndianUtil.readSwappedShort( FULL_BYTES, 0 ) );
        assertEqualArray( "SWAPPED_SHORT", FULL_BYTES, data, 2 );
        EndianUtil.writeSwappedUnsignedShort( data, 0, EndianUtil.readSwappedUnsignedShort( FULL_BYTES, 0 ) );
        assertEqualArray( "Unsigned SWAPPED_SHORT", FULL_BYTES, data, 2 );
        EndianUtil.writeSwappedInteger( data, 0, EndianUtil.readSwappedInteger( FULL_BYTES, 0 ) );
        assertEqualArray( "SWAPPED_INTEGER", FULL_BYTES, data, 4 );
        EndianUtil.writeSwappedUnsignedInteger( data, 0, EndianUtil.readSwappedUnsignedInteger( FULL_BYTES, 0 ) );
        assertEqualArray( "Unsigned SWAPPED_INTEGER", FULL_BYTES, data, 4 );
        EndianUtil.writeSwappedLong( data, 0, EndianUtil.readSwappedLong( FULL_BYTES, 0 ) );
        assertEqualArray( "SWAPPED_LONG", FULL_BYTES, data, 8 );
        EndianUtil.writeSwappedFloat( data, 0, EndianUtil.readSwappedFloat( FULL_BYTES, 0 ) );
        assertEqualArray( "SWAPPED_FLOAT", FULL_BYTES, data, 4 );
        EndianUtil.writeSwappedDouble( data, 0, EndianUtil.readSwappedDouble( FULL_BYTES, 0 ) );
        ...
        throws Exception
    {
        ...
        assertEquals( "Stream reading SHORT", SHORT_VALUE, EndianUtil.readSwappedShort( input ) );

        input = new ByteArrayInputStream( US_SHORT_BYTES );
        assertEquals( "Stream reading US_SHORT", US_SHORT_VALUE, EndianUtil.readSwappedUnsignedShort( input ) );

        input = new ByteArrayInputStream( INTEGER_BYTES );
        assertEquals( "Stream reading INTEGER", INTEGER_VALUE, EndianUtil.readSwappedInteger( input ) );

        input = new ByteArrayInputStream( US_INTEGER_BYTES );
        assertEquals( "Stream reading US_INTEGER", US_INTEGER_VALUE, EndianUtil.readSwappedUnsignedInteger( input ) );

        input = new ByteArrayInputStream( LONG_BYTES );
        assertEquals( "Stream reading LONG", LONG_VALUE, EndianUtil.readSwappedLong( input ) );

        input = new ByteArrayInputStream( FLOAT_BYTES );
        assertTrue( "Stream reading FLOAT", FLOAT_VALUE == EndianUtil.readSwappedFloat( input ) );

        input = new ByteArrayInputStream( DOUBLE_BYTES );
        assertTrue( "Stream reading FLOAT", DOUBLE_VALUE == EndianUtil.readSwappedDouble( input ) );
    }
        ...
        throws Exception
    {
        ...
        assertEqualArray( "US_SHORT_VALUE", US_SHORT_BYTES, output.toByteArray() );
        output.reset();

        EndianUtil.writeSwappedInteger( output, INTEGER_VALUE );
        assertEqualArray( "INTEGER_VALUE", INTEGER_BYTES, output.toByteArray() );
        output.reset();

        EndianUtil.writeSwappedUnsignedInteger( output, US_INTEGER_VALUE );
        assertEqualArray( "US_INTEGER_VALUE", US_INTEGER_BYTES, output.toByteArray() );
        output.reset();
    ...
    private void assertEqualArray( final String array, final byte[] expected, final byte[] data )
    {
        assertEqualArray( array, expected, data, expected.length );
    }

    private void assertEqualArray( final String array, final byte[] expected, final byte[] data, final int length )
    {
    ...
    protected void displayByteData( String label, int size, final byte[] data )
    {
